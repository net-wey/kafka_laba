...
type PostQuery struct {
	...
	withLikes    *PostLikeQuery
	withViews    *PostViewQuery
...
func (pq *PostQuery) QueryLikes() *PostLikeQuery {
	query := &PostLikeQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) QueryViews() *PostViewQuery {
	query := &PostViewQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) WithLikes(opts ...func(*PostLikeQuery)) *PostQuery {
	query := &PostLikeQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) WithViews(opts ...func(*PostViewQuery)) *PostQuery {
	query := &PostViewQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withLikes; query != nil {
		if err := pq.loadLikes(ctx, query, nodes,
			func(n *Post) { n.Edges.Likes = []*PostLike{} },
			func(n *Post, e *PostLike) { n.Edges.Likes = append(n.Edges.Likes, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withViews; query != nil {
		if err := pq.loadViews(ctx, query, nodes,
			func(n *Post) { n.Edges.Views = []*PostView{} },
			func(n *Post, e *PostView) { n.Edges.Views = append(n.Edges.Views, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PostQuery) loadLikes(ctx context.Context, query *PostLikeQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostLike)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.PostLike(func(s *sql.Selector) {
...
func (pq *PostQuery) loadViews(ctx context.Context, query *PostViewQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostView)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.PostView(func(s *sql.Selector) {
		s.Where(sql.InValues(post.ViewsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
